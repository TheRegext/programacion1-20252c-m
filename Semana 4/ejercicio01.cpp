#include <iostream>
using namespace std;

/***
  for como el while se usa para repetir instrucciones
  while siempre que no sepamos la cantidad de repeticiones

  for cuando sabemos la cantidad de repeticiones o querramos recorrer

*/

int main()
{

  int numero;

  cout << "Ingrese numero para la tabla: ";
  cin >> numero;

  for(int i=1; i<=10; i++){
    cout << numero <<" x " << i << " = "<< numero * i << endl; 
  }
  
  // cout << numero <<" x v  = "<< numero * v << endl; donde v es un numero entre 1 y 10
  /*
  cout << numero <<" x 1  = "<< numero * 1 << endl;
  cout << numero <<" x 2  = "<< numero * 2 << endl;
  cout << numero <<" x 3  = "<< numero * 3 << endl;
  cout << numero <<" x 4  = "<< numero * 4 << endl;
  cout << numero <<" x 5  = "<< numero * 5 << endl;
  cout << numero <<" x 6  = "<< numero * 6 << endl;
  cout << numero <<" x 7  = "<< numero * 7 << endl;
  cout << numero <<" x 8  = "<< numero * 8 << endl;
  cout << numero <<" x 9  = "<< numero * 9 << endl;
  cout << numero <<" x 10 = "<< numero * 10 << endl;
*/


  return 0;
}
